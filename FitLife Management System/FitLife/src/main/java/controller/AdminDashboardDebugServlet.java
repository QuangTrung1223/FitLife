...
@WebServlet(name = "AdminDashboardDebugServlet", urlPatterns = {"/admin-dashboard-debug"})
public class AdminDashboardDebugServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            System.out.println("=== [AdminDashboardDebugServlet] Debug Info ===");
            ...
            if (sessionLogin == null || userRole == null || !"admin".equalsIgnoreCase(userRole)) {
                System.out.println("[AdminDashboardDebugServlet] Redirecting to login - not admin");
                response.sendRedirect(request.getContextPath() + "/login.jsp");
                return;
            }

            System.out.println("[AdminDashboardDebugServlet] Admin access granted, fetching data...");
            ...
            System.out.println("=== [AdminDashboardDebugServlet] Fetching data ===");
            ...
            System.out.println("[AdminDashboardDebugServlet] Forwarding to debug JSP...");
            // Forward to debug JSP
            request.getRequestDispatcher("/admin-dashboard-debug.jsp").forward(request, response);
            System.out.println("[AdminDashboardDebugServlet] Forward completed successfully");
            
        } catch (Exception e) {
            System.err.println("=== [AdminDashboardDebugServlet] ERROR ===");
            e.printStackTrace();
            
            // Send error response
            response.setContentType("text/html");
            response.getWriter().println("<html><body>");
            response.getWriter().println("<h1>Admin Dashboard Debug Error</h1>");
